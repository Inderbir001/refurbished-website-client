...
const schema = z.object({ name: z.string().trim().min(2).max(100), phone: z.union([z.literal(""), indianPhone]).optional(), currentPassword: z.string().optional(), newPassword: z.string().min(8).max(72).optional() }).refine((value) => !value.newPassword || Boolean(value.currentPassword), { message: "Current password is required to set a new password." });
