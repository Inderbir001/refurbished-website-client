...
import { addCartItem, cartTotals, changeCartItem } from "@/lib/services/cart-service";
const guestCookie = "guest_cart";
const postSchema = z.object({ productId: z.string().cuid(), variantId: z.string().cuid().optional(), quantity: z.number().int().min(1).max(10) });
...
export async function POST(request: NextRequest) { try { const identity = await actor(request); const cart = await addCartItem(identity.actor, postSchema.parse(await request.json())); return response(cart, identity.actor.userId, identity.freshGuest ? identity.actor.sessionToken : undefined); } catch (error) { return apiError(error); } }
