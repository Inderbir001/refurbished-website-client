...
const bodySchema = z.object({ name: z.string().trim().min(2).max(100), phone: indianPhone, email: optionalEmail, password: z.string().min(8).max(72) });
export async function POST(request: Request) {
  try {
    ...
    let user;
    try { user = await db.user.create({ data: { name: input.name, phone: input.phone, email: input.email ?? null, passwordHash: await bcrypt.hash(input.password, 12) } }); }
