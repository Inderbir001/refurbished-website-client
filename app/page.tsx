    ...
    if (section.type === HomepageSectionType.HERO) return <div key={section.id}>
      <section className="offer-hero">
        <div className="offer-main">
          ...
          <span className="offer-tag">● LIVE OFFERS</span>
          <h1>{custom(section.title, "") ? custom(section.title, "") : maxPercent > 0 ? <>Up to <em>{maxPercent}% off</em></> : <>Great deals on <em>electronics</em></>}</h1>
          ...
          <div className="offer-stats"><span><b>{onOffer.length}</b> products on offer</span><span><b>{store?.settings?.returnDays ?? 7} days</b> easy returns</span><span><b>Free delivery</b> over {inr(store?.settings?.freeShippingThreshold ?? 49900)}</span></div>
        </div>
      ...
      {liveCoupons.length > 0 && <div className="coupon-strip">{liveCoupons.map((coupon) => { const copy = couponCopy(coupon); return <article className="coupon" key={coupon.id}><div><strong>{copy.value}</strong><small>{copy.details}</small></div><CopyCode code={coupon.code} /></article>; })}</div>}
      {budgetTiles.length > 0 && <section className="offer-section"><div className="offer-head"><div><p className="eyebrow">SHOP BY BUDGET</p><h2>Find something in your range</h2></div></div><div className="budget-row">{budgetTiles.map((tile) => <Link key={tile.limit} href={`/products?max=${tile.limit}&sort=price-asc`}><small>UNDER</small><strong>{inr(tile.limit * 100)}</strong><span>{tile.count} products →</span></Link>)}</div></section>}
    </div>;
    if (section.type === HomepageSectionType.FEATURED_PRODUCTS) return <div key={section.id}>
      <section className="offer-section"><div className="offer-head"><div><p className="eyebrow">BIGGEST DISCOUNTS</p><h2>{custom(section.title, "Top deals right now")}</h2></div><Link className="text-link" href="/products?sort=price-asc">See all deals →</Link></div>{topDeals.length ? <div className="product-grid">{topDeals.map((product) => <ProductCard key={product.id} product={product} />)}</div> : <div className="empty-state"><h3>New offers are on the way.</h3><p><Link className="text-link" href="/products">Browse the catalog</Link> in the meantime.</p></div>}</section>
      {refurbished.length > 0 && <section className="refurb-band"><div className="offer-head"><div><p className="eyebrow">CERTIFIED REFURBISHED</p><h2>Same tech, {refurbSavings ? `up to ${refurbSavings}% ` : ""}less</h2></div><Link className="text-link" href="/products?condition=REFURBISHED">Shop refurbished →</Link></div><div className="product-grid">{refurbished.slice(0, 4).map(({ product }) => <ProductCard key={product.id} product={product} />)}</div></section>}
    </div>;
    if (section.type === HomepageSectionType.CATEGORIES) return categoryOffers.length ? <section className="offer-section" key={section.id}><div className="offer-head"><div><p className="eyebrow">OFFERS BY CATEGORY</p><h2>{custom(section.title, "Pick a category, pick a saving")}</h2></div></div><div className="cat-offers">{categoryOffers.map((category) => <Link key={category.slug} href={`/category/${category.slug}`}><small>{category.percent > 0 ? "UP TO" : "STARTING"}</small><strong>{category.percent > 0 ? `${category.percent}% off` : inr(category.from)}</strong><span>{category.name}</span><em>from {inr(category.from)} · {category.count} {category.count === 1 ? "item" : "items"}</em></Link>)}</div></section> : null;
    if (section.type === HomepageSectionType.COLLECTION) return <div key={section.id}>{collections.filter((collection) => collection.products.length).slice(0, 2).map((collection) => <section className="offer-section" key={collection.id}><div className="offer-head"><div><p className="eyebrow">CURATED OFFER</p><h2>{collection.name}</h2></div><Link className="text-link" href="/products">See all →</Link></div><div className="product-grid">{collection.products.map(({ product }) => <ProductCard key={product.id} product={product} />)}</div></section>)}</div>;
    if (section.type === HomepageSectionType.BRANDS) return brandOffers.length ? <section className="offer-section" key={section.id}><div className="offer-head"><div><p className="eyebrow">BRAND OFFERS</p><h2>{custom(section.title, "Top brands, better prices")}</h2></div></div><div className="brand-offers">{brandOffers.map((brand) => <Link key={brand.slug} href={`/products?brand=${brand.slug}`}><b>{brand.name}</b><small>{brand.percent > 0 ? `up to ${brand.percent}% off` : `${brand.count} products`}</small></Link>)}</div></section> : null;
