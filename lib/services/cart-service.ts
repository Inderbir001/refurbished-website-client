...
const cartInclude = { items: { include: { product: { include: { images: { orderBy: { position: "asc" }, take: 1 }, collections: { select: { collectionId: true } } } }, variant: true } } } satisfies Prisma.CartInclude;
// The cart record only (one cheap query). Loading the items with their products, photos and variants is several queries,
// so it happens once per request, at the end.
async function ensureCart(actor: CartActor) {
  ...
  return db.cart.upsert({ where, update: {}, create: actor.userId ? { userId: actor.userId } : { sessionToken: actor.sessionToken! } });
}
