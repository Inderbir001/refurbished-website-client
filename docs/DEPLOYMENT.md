...
## How pages get their data (why the site is fast on Render)

Storefront pages (header, home, products, category, product) ask the backend for **one** answer each through
`/api/internal/page` (see `lib/page-data.ts`), instead of one round trip per piece of data. The backend keeps the public
parts in memory for 20 seconds (`STOREFRONT_CACHE_SECONDS`, defaults to 20 when unset) and forgets them the moment an
admin saves a change, so an edit shows on the store within moments, not after a stale 20-second wait.

Render's nearest region is Singapore, one hop from Supabase's Mumbai database; that hop (not the app) is the remaining
floor on page speed. If that ever needs closing, the backend is a plain Next.js app and can be redeployed to any host
with a Mumbai region without other changes — not needed today.
